...
export default function VerifyCertificate() {
    ...
    async (qrCodeData) => {
      ...
      try {
        const res = await fetch(
          "http://localhost:4000/certificates/verify-data",
          {
            method: "POST",
  ...
  const LightCard = ({
    ...
    <button
      onClick={onClick}
      className={`group bg-white border ${colSpan}  border-blue-400 rounded-2xl p-6 hover:shadow-lg transition-all`}
    >
      <div className={`flex justify-between`}>
        <div
          className={`w-12 h-12 rounded-xl flex items-center  justify-center bg-gray-50 ${accentColor}`}
        >
          <Icon className="w-6 h-6 text-blue-400 group-hover:text-blue-600" />
        </div>
        <ArrowRight className="w-5 h-5 text-blue-700 opacity-0 group-hover:opacity-100 transition-all" />
      </div>
      <h3 className="text-lg font-semibold text-gray-400 group-hover:text-blue-600 mt-4">
        {title}
      </h3>
  ...
  return (
    <div className=" w-full h-screen flex  items-center  px-6  py-10 font-sans">
      <div className="w-7xl  mx-auto">
          ...
          <div className=" w-full text-gray-900 font-sans selection:bg-blue-100 flex items-center justify-center  relative overflow-hidden">
            ...
            <div className="relative z-10 w-full max-w-4xl">
              ...
              <div className="bg-white/80 backdrop-blur-xl border border-white/50 shadow-[0_8px_30px_rgb(0,0,0,0.04)] rounded-[2rem] p-8 ">
                {/* Header */}
                <div className="mb-10">
                  <h2 className="text-3xl flex justify-between font-bold tracking-tight text-blue-600">
                  ...
                  <p className="text-gray-500 mt-2 text-base">
                    Select how you would like to verify the certificate
                    credentials.
                  </p>
                </div>
        ...
        {scanMode === "pdf" && <VerifyPdf verify={verify} />}
        ...
        <div className="mt-8 text-center">
          <div className="inline-flex items-center gap-2 px-4 py-2 bg-white border rounded-full shadow-sm">
            ...
            <p className="text-sm text-gray-600">
              Use the QR from the original certificate
            </p>
          </div>
        </div>
      </div>
