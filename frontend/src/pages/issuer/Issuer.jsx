...
import { 
  Search, Plus, LogOut, Shield, Award, FileText, 
  Calendar, Mail, User, Building, Grid, 
  List, Filter, Eye, MoreVertical, CheckCircle
} from "lucide-react";
    ...
    const loadData = async () => {
      ...
      try {
        if (details && details.email) {
          ...
          const certis = await axios.get(`http://localhost:4000/issuer/${details.id}/certificates`);
      ...
      <aside className="fixed left-0 top-0 h-full w-64  border-r border-gray-200 flex flex-col z-20">
        ...
        <div className="p-6">
          <div className="flex items-center gap-3">
            ...
            <span className="font-semibold text-xl text-black tracking-tight">QR Certify</span>
          </div>
        </div>
          ...
          {issuerProfile && (
            <div className="flex items-center gap-3 mb-3 p-3 bg-gray-700 rounded-lg">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-gray-400 truncate">{issuerProfile.issuerTitle}</p>
              </div>
            </div>
          )}
          <button
        ...
        <div className="flex flex-col justify-center space-y-1">
          ...
          <div className="pt-2 flex flex-col gap-1.5">
             <div className="flex items-center gap-2 text-sm text-zinc-500">
              ...
              <span className="text-zinc-300 font-medium">{issuerProfile.issuerName}</span>
            </div>
            <div className="flex items-center gap-2 text-sm text-zinc-500">
              <Mail className="w-4 h-4" />
              <span className="text-zinc-400 truncate">{issuerProfile.email}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="flex gap-4 shrink-0 ">
        ...
        <div className="bg-zinc-900 justify-between flex flex-col text-white border border-zinc-800 rounded-2xl w-40 h-40 shadow-lg">
          <div className="flex items-center justify-center border-b border-zinc-800 p-3 bg-zinc-800/50 rounded-t-2xl">
            <span className="text-sm font-semibold text-zinc-300 uppercase tracking-wider">Total</span>
          </div>
        ...
        <div className="bg-zinc-900 justify-between flex flex-col text-white border border-zinc-800 rounded-2xl w-40 h-40 shadow-lg">
           <div className="flex items-center justify-center border-b border-zinc-800 p-3 bg-zinc-800/50 rounded-t-2xl">
            <span className="text-sm font-semibold text-zinc-300 uppercase tracking-wider">This Month</span>
          </div>
              ...
              {filteredCertificates.map((cert) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-4 text-xs text-gray-500 mb-4">
                    ...
                    <div className="flex items-center gap-1">
                      <span className="font-mono text-xs">{cert._id?.slice(-4)}</span>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-between pt-4 border-t border-gray-100">
                    ...
                    <button 
                      onClick={() => navigate(`/view-certificate`, {state: {cert}})}
                    ...
                    <tr key={cert._id} className="hover:bg-gray-50 transition-colors">
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center gap-3">
                          ...
                          <span className="font-semibold text-black">{cert.recipientName}</span>
                        </div>
                      </td>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className="font-mono text-xs text-gray-600">{cert._id?.slice(-8)}</span>
                      </td>
          ...
          {!isLoading && filteredCertificates.length === 0 && (
            <div className="bg-white border border-gray-200 rounded-2xl p-16 text-center">
              ...
              <h3 className="text-lg font-semibold text-black mb-2">No certificates found</h3>
              <p className="text-gray-600 mb-6">
                {searchQuery ? "Try adjusting your search" : "Get started by issuing your first certificate"}
              </p>
              {!searchQuery && (
                <button
                  onClick={handleIssueNew}
