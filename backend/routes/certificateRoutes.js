...
router.post("/extract-qr", upload.single("qrImage"), async (req, res) => {
  ...
  try {
    ...
    if (!qr) {
      return res
        .status(400)
        .json({ success: false, message: "No QR code detected" });
    }
...
router.post("/generate-qr", async (req, res) => {
  try {
    ...
    if (!student || !issuer) {
      return res
        .status(400)
        .json({ success: false, message: "Student or Issuer not found" });
    }
    ...
    if (remove_request) {
      ...
      const deletedRequest = await CertiRequest.findByIdAndDelete(
        remove_request
      );
...
router.post("/verify-data", async (req, res) => {
  try {
    ...
    if (!cert) {
      return res
        .status(404)
        .json({ valid: false, message: "Certificate not found in Registry" });
    }
    ...
    if (!blockchainHash || blockchainHash === "") {
      return res
        ...
        .json({
          valid: false,
          message: "No blockchain record found for this ID",
        });
    }
    ...
    return res
      .status(500)
      .json({ valid: false, message: "Server error during verification." });
  }
});
